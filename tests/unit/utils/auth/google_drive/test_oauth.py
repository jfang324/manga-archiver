...
def _configure_auth_pipeline(
    ...
    mock_token_store.return_value.load = AsyncMock(return_value=None)
    mock_token_store.return_value.save = AsyncMock()
...
class TestHandleAuthLogin:
    ...
    async def test_login_saves_token_and_returns_0_on_success(
        self,
        mock_get_path: MagicMock,
        mock_exists: MagicMock,
        mock_load_creds: MagicMock,
        ...
        mock_token_store.return_value.save.assert_awaited_once()
    ...
    @patch("src.manga_archiver.utils.auth.google_drive.oauth.os.path.exists")
    @patch("src.manga_archiver.utils.auth.google_drive.oauth._get_credentials_path")
    async def test_login_returns_exit_auth_login_failed_on_network_error(
        self,
        mock_get_path: MagicMock,
        mock_exists: MagicMock,
        mock_load_creds: MagicMock,
    ...
    @patch("src.manga_archiver.utils.auth.google_drive.oauth.os.path.exists")
    @patch("src.manga_archiver.utils.auth.google_drive.oauth._get_credentials_path")
    async def test_login_returns_exit_auth_login_failed_on_http_error(
        self,
        mock_get_path: MagicMock,
        mock_exists: MagicMock,
        mock_load_creds: MagicMock,
...
class TestHandleAuthLogout:
    ...
    async def test_logout_deletes_token_and_returns_0_on_success(
        self,
        mock_post: MagicMock,
        mock_token_store: MagicMock,
    ) -> None:
        mock_token_store.return_value.load = AsyncMock(
            return_value={"refresh_token": "test_refresh_token"}
        )
        mock_token_store.return_value.delete = AsyncMock()
        ...
        assert result == EXIT_SUCCESS
        mock_token_store.return_value.delete.assert_awaited_once()

    @patch("src.manga_archiver.utils.auth.google_drive.oauth.GoogleDriveTokenStore")
    @patch("src.manga_archiver.utils.auth.google_drive.oauth.requests.post")
    async def test_logout_returns_exit_auth_logout_failed_on_network_error(
        self,
        mock_post: MagicMock,
        mock_token_store: MagicMock,
        capsys,
