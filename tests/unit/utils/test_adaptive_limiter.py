...
class TestAdaptiveLimiter:
    async def _hold_base_slot(self, limiter: AdaptiveLimiter) -> HeldSlot:
        acquired = asyncio.Event()
        release = asyncio.Event()
    ...
    async def _start_acquire_probe(self, limiter: AdaptiveLimiter) -> AcquireProbe:
        acquired = asyncio.Event()
    ...
    def test_init_rejects_invalid_token_ttl(self) -> None:
        with pytest.raises(ValueError, match="token_ttl_seconds must be greater than 0"):
            AdaptiveLimiter(base_limit=1, token_ttl_seconds=0)

    async def test_acquire_allows_work_inside_context(self) -> None:
        limiter = AdaptiveLimiter(base_limit=1)
        did_work = False
